  ...
  useEffect(() => {
    const loadReading = async () => {
      try {
        ...
        } else if (fromStart || fromRitual || fromHistory) {
          const readingResult = sessionStorage.getItem('readingResult');
          
          if (readingResult) {
            try {
              const result = JSON.parse(readingResult);
              console.log('从历史记录加载的解读数据:', result);
              
              // 验证数据格式
              if (!result || (!result.overall && !result.readingResults)) {
                console.error('解读数据格式不正确:', result);
                setError('解读数据格式不正确');
                return;
              }
