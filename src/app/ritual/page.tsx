...
function RitualPageContent() {
  ...
  // 调试信息
  console.log('Ritual page - spread:', spread);
  console.log('Ritual page - question:', question);
  console.log('Ritual page - searchParams:', Object.fromEntries(searchParams.entries()));
  
  // 验证spread参数的有效性
  const validSpreads: SpreadType[] = ['single', 'situation-action-outcome', 'five-card'];
  const validatedSpread = validSpreads.includes(spread) ? spread : 'single';
  ...
  const cardCount = getCardCount(validatedSpread);
  ...
  const [autoShuffleStarted, setAutoShuffleStarted] = useState(false);
  ...
  // 自动开始洗牌
  useEffect(() => {
    if (!autoShuffleStarted) {
      setAutoShuffleStarted(true);
      // 延迟0.5秒后自动开始洗牌
      setTimeout(() => {
        handleAutoShuffle();
      }, 500);
  ...
  const handleAutoShuffle = () => {
    if (isShuffling || isShuffled) return;
  ...
  const handleCardClick = (card: SelectedCard) => {
    ...
    // 洗牌前：如果还没开始自动洗牌，点击任意卡牌触发洗牌动画
    if (!isShuffled && !autoShuffleStarted) {
  ...
  const handleStartReading = async () => {
    ...
    // 构建卡牌数据用于解读
    const positions =
      validatedSpread === 'single'
        ? ['当前状况']
        : validatedSpread === 'situation-action-outcome'
        ? ['现状', '行动', '结果']
        : ['过去', '现在', '未来', '建议', '结果'];
          ...
          {selectedSlots.filter(Boolean).length === cardCount && (
            <div className="absolute top-1/6 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-none" style={{ marginLeft: '5px' }}> {/* 向左移动，从60px减少到20px */}
              <button
                ...
                className={`pointer-events-auto px-3 py-1.5 rounded font-medium text-sm transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 bg-white/70 border border-white/60 shadow-lg hover:shadow-xl hover:bg-white/80 active:bg-white/90 ${
                  isAnimating 
